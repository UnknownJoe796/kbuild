...
class AndroidProject(
    ...
    val androidDir: File = kmpProject.projectRoot.resolve("android")
) {
     ...
     */
    fun scaffold(
        appName: String = kmpProject.name,
        mainActivity: String = ".MainActivity"
    ): ScaffoldResult {
        require(KmpTarget.Jvm in kmpProject.targets) {
        ...
        val srcDir = kmpProject.projectRoot.resolve("src/jvmMain/kotlin")
        srcDir.mkdirs()
        ...
        activityPackageDir.resolve("$activityClassName.kt").writeText("""
    ...
    private fun compileKotlinForAndroid(rJavaDir: File? = null): File {
        ...
        // Get classpath from KmpProject dependencies + android.jar
        val baseClasspath = kmpProject.dependencies.resolveJvmClasspath()
        val classpath = baseClasspath + setOf(androidJar)

        // Include R.java directory in source roots if it exists
        val sourceDirs = kmpProject.getSourcesForTarget(KmpTarget.Jvm).toMutableSet()
        if (rJavaDir != null && rJavaDir.exists()) {
            sourceDirs.add(rJavaDir)
        }

        val compile = KotlinJvmCompile(
            name = kmpProject.name,
            sourceRoots = { sourceDirs },
            classpathJars = { classpath },
            cache = buildDir.resolve("kotlin/android/cache"),
    ...
    fun build(config: BuildConfig = BuildConfig()): BuildResult {
        ...
        println("Building Android APK: ${kmpProject.name}")
        ...
        outputsDir.mkdirs()
        val alignedApk = outputsDir.resolve("${kmpProject.name}-aligned.apk")
        builder.align(baseApk, alignedApk)
        println("  Aligned: $alignedApk")

        // Step 7: Sign APK
        println("Step 7/7: Signing APK...")
        val keystore = ApkBuilder.debugKeystore(kmpProject.projectRoot)
        val signedApk = outputsDir.resolve(
            if (config.debug) "${kmpProject.name}-debug.apk"
            else "${kmpProject.name}-release.apk"
        )
    ...
    fun logcat(lines: Int = 100): String {
        ...
        return if (pid.isNotEmpty()) {
            ...
            result.output.lines()
                .filter { it.contains(packageName) || it.contains(kmpProject.name) }
                .joinToString("\n")
        }
    }
...
/**
 * DSL for creating an Android project from a KMP project.
 */
fun KmpProject.androidProject(
    packageName: String,
    minSdk: Int = 24,
    targetSdk: Int = 34,
    versionCode: Int = 1,
