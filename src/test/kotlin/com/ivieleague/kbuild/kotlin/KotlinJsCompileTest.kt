...
class KotlinJsCompileTest {
    ...
    fun `experimental incremental compilation with ChangedFiles hack`() = runBlocking {
        ...
        println("\n=== EXPERIMENTAL INCREMENTAL COMPILATION TEST ===")

        // First build
        println("\n1. First build (empty cache)...")
        val firstResult = compileToKlibIncrementalExperimental(
            name = "experimental-test",
            sourceRoots = setOf(srcDir),
            libraries = jsStdlib.toList(),
            cache = cacheDir,
            outputDir = outputDir,
            arguments = {}
        )
        println("   Result: ${if (firstResult != null) "SUCCESS" else "FAILED"}")
        assertTrue(firstResult != null && firstResult.exists(), "First build should succeed")

        // Second build (no changes)
        println("\n2. Second build (no changes)...")
        val secondResult = compileToKlibIncrementalExperimental(
            name = "experimental-test",
            sourceRoots = setOf(srcDir),
            libraries = jsStdlib.toList(),
            cache = cacheDir,
        ...
        println("   Result: ${if (secondResult != null) "SUCCESS" else "FAILED"}")
        assertTrue(secondResult != null && secondResult.exists(), "Second build should succeed")

        // Third build (with change)
        Thread.sleep(50)
        srcDir.resolve("Utils.kt").writeText("""
            fun greet(name: String) {
                println("Hi, ${'$'}name!")  // Changed
            }
        """.trimIndent())

        println("\n3. Third build (file changed)...")
        val thirdResult = compileToKlibIncrementalExperimental(
            name = "experimental-test",
            sourceRoots = setOf(srcDir),
            libraries = jsStdlib.toList(),
            cache = cacheDir,
        ...
        println("   Result: ${if (thirdResult != null) "SUCCESS" else "FAILED"}")
        assertTrue(thirdResult != null && thirdResult.exists(), "Third build should succeed")
